...
public:
    ...
    // THE MAGIC FUNCTION: Segment Tree Walk
    int searchLeftmost(int start, int end, int target_height, int i, int l, int r, 
                       const vector<int>& segmentTree, const vector<int>& heights) {
        
        // 1. Out of bounds OR the maximum in this range is too short
        if (l > end || r < start || heights[segmentTree[i]] <= target_height) {
            return -1;
        }
        
        // 2. We reached a valid leaf
        if (l == r) {
            return l;
        }
        ...
        // 3. Try the LEFT child first (because we want the leftmost valid index)
        int left_result = searchLeftmost(start, end, target_height, 2 * i + 1, l, mid, segmentTree, heights);
        
        // If we found a valid building on the left, return it immediately!
        if (left_result != -1) {
            return left_result;
        }
        
        // 4. Otherwise, the answer must be on the RIGHT child
        return searchLeftmost(start, end, target_height, 2 * i + 2, mid + 1, r, segmentTree, heights);
    }
    ...
    vector<int> leftmostBuildingQueries(vector<int>& heights, vector<vector<int>>& queries) {
        ...
        for(auto& query: queries){
            ...
            // Base case: they can meet at max_i
            if(min_i == max_i || heights[max_i] > heights[min_i]){
                result.push_back(max_i);
                continue;
            }

            // The target height we must strictly exceed
            int target = heights[min_i]; 
            
            // Just one O(log N) call to the tree instead of a while loop!
            int ans = searchLeftmost(max_i + 1, n - 1, target, 0, 0, n - 1, segmentTree, heights);
